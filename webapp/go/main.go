...
func initialize(c echo.Context) error {
	...
	paths := []string{
		...
		filepath.Join(sqlDir, "3_AlterTableData.sql"),
	}
...
func postChair(c echo.Context) error {
	...
	for _, row := range records {
		...
		values := fmt.Sprintf(`("%v","%v","%v","%v","%v","%v","%v","%v","%v","%v","%v","%v","%v")`, id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock)
		bulk = append(bulk, values)
	}

	query := "INSERT INTO chair(id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock) VALUES " + strings.Join(bulk, ", ")
	_, err = tx.Exec(query)
	if err != nil {
...
func postEstate(c echo.Context) error {
	...
	for _, row := range records {
		...
		values := fmt.Sprintf(`("%v","%v","%v","%v","%v","%v","%v","%v","%v","%v","%v","%v")`, id, name, description, thumbnail, address, latitude, longitude, rent, doorHeight, doorWidth, features, popularity)
		bulk = append(bulk, values)
	}
	query := "INSERT INTO estate(id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features, popularity) VALUES " + strings.Join(bulk, ", ")
	_, err = tx.Exec(query)
	if err != nil {
